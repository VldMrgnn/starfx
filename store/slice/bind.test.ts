...
import type { PipeCtx, LoaderCtx, ActionWithPayload, Result, ParallelRet } from "../../mod.ts";
...
const removeUser = thunks.create<TUser>(
	...
	asserts.assertEquals(store.getState()[REPO_DS_USER], { 1: { id: '1', userName: "user1" } });
	
	store.dispatch(addUser({ id: '2', userName: "user2" }));
	asserts.assertEquals(store.getState()[REPO_DS_USER], { 1: { id: '1', userName: "user1" } , 2: { id: '2', userName: "user2" }});
	
	store.dispatch(patchUser({ id: '2', userName: "user2-updated" }));
	asserts.assertEquals(store.getState()[REPO_DS_USER], { 1: { id: '1', userName: "user1" } , 2: { id: '2', userName: "user2-updated" }});
